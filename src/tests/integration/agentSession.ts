  ...
  try {
    ...
    for await (const message of queryResult) {
      ...
      if (message.type === 'assistant') {
        ...
        const content = message.message.content as Array<{ type: string; text?: string; name?: string; input?: unknown }>;
        ...
        const toolUseBlocks = content
          .filter((block): block is { type: 'tool_use'; name: string; input: unknown } =>
        ...
        if (message.subtype !== 'success') {
          ...
          if ('errors' in message && message.errors) {
            const blockError = message.errors.find(e =>
              e.toLowerCase().includes('block') ||
        ...
        if (message.subtype === 'hook_response') {
          ...
          const hookMsg = message as { stdout: string; stderr: string };
          try {
