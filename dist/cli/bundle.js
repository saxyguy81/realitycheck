...
var LimitsConfigSchema = external_exports.object({
  ...
  maxTotalAttempts: external_exports.number().min(1).max(200).default(50),
  noProgressThreshold: external_exports.number().min(1).max(50).default(5)
});
var StorageConfigSchema = external_exports.object({
  directory: external_exports.string().default(".claude/realitycheck"),
  ledgerFilename: external_exports.string().default("task_ledger.json"),
  archiveCorrupted: external_exports.boolean().default(true)
});
var GitConfigSchema = external_exports.object({
  enabled: external_exports.boolean().default(true),
  captureBaseline: external_exports.boolean().default(true),
  includeDiff: external_exports.boolean().default(true)
});
var PerformanceConfigSchema = external_exports.object({
  fingerprintOnToolUse: external_exports.boolean().default(false),
  lazyTranscriptLoad: external_exports.boolean().default(true)
});
var DebugConfigSchema = external_exports.object({
  enabled: external_exports.boolean().default(false),
  logLevel: external_exports.enum(["error", "warn", "info", "debug"]).default("info"),
  dryRun: external_exports.boolean().default(false)
});
var RealityCheckConfigSchema = external_exports.object({
  ...
  for (const location of CONFIG_LOCATIONS) {
    ...
    if (existsSync(configPath)) {
      try {
        ...
        console.warn(
          `[RealityCheck] Failed to load config from ${configPath}: ${error instanceof Error ? error.message : "Unknown error"}`
        );
      }
    }
  }
...
function getStoragePath(config, projectDir) {
  return join(projectDir, config.storage.directory);
}
function getLedgerPath(config, projectDir) {
...
// src/types/index.ts
var BaseHookInputSchema = external_exports.object({
  session_id: external_exports.string(),
  transcript_path: external_exports.string()
});
var UserPromptSubmitInputSchema = BaseHookInputSchema.extend({
  hook_type: external_exports.literal("UserPromptSubmit"),
  prompt: external_exports.string()
});
var StopHookInputSchema = BaseHookInputSchema.extend({
  hook_type: external_exports.literal("Stop"),
  stop_hook_active: external_exports.boolean().default(false)
});
var PostToolUseInputSchema = BaseHookInputSchema.extend({
  hook_type: external_exports.literal("PostToolUse"),
  tool_name: external_exports.string(),
  tool_input: external_exports.unknown(),
  tool_output: external_exports.unknown().optional()
});
var SessionStartInputSchema = BaseHookInputSchema.extend({
  hook_type: external_exports.literal("SessionStart")
});
var HookInputSchema = external_exports.discriminatedUnion("hook_type", [
...
var DirectiveStatusSchema = external_exports.enum([
  "active",
  ...
  "abandoned"
  // User gave up or changed direction
]);
var DirectiveTypeSchema = external_exports.enum([
  "initial",
...
var DirectiveSchema = external_exports.object({
  id: external_exports.string().uuid(),
  rawText: external_exports.string(),
  normalizedIntent: external_exports.string().optional(),
...
var AcceptanceCriterionSchema = external_exports.object({
  id: external_exports.string().uuid(),
  directiveId: external_exports.string().uuid(),
...
var VerdictStatusSchema = external_exports.enum([
  ...
  "blocked",
  // Task cannot proceed (needs user input)
  "error"
  // Judge encountered an error
]);
var StopAttemptSchema = external_exports.object({
  id: external_exports.string().uuid(),
  timestamp: external_exports.string().datetime(),
  verdict: VerdictStatusSchema,
  reason: external_exports.string(),
  fingerprintBefore: external_exports.string().optional(),
  fingerprintAfter: external_exports.string().optional(),
  criteriaEvaluated: external_exports.array(external_exports.object({
    criterionId: external_exports.string().uuid(),
    passed: external_exports.boolean(),
    notes: external_exports.string().optional()
  })).optional()
});
var GitBaselineSchema = external_exports.object({
...
var WorkspaceFingerprintSchema = external_exports.object({
  hash: external_exports.string(),
  timestamp: external_exports.string().datetime(),
  afterCommand: external_exports.string().optional()
});
var TaskLedgerSchema = external_exports.object({
  version: external_exports.literal(1),
  sessionId: external_exports.string(),
  createdAt: external_exports.string().datetime(),
  updatedAt: external_exports.string().datetime(),
  directives: external_exports.array(DirectiveSchema),
  criteria: external_exports.array(AcceptanceCriterionSchema),
  stopAttempts: external_exports.array(StopAttemptSchema),
  gitBaseline: GitBaselineSchema.optional(),
  fingerprints: external_exports.array(WorkspaceFingerprintSchema)
});
var ProgressTrendSchema = external_exports.enum([
  "improving",
  // Making progress toward completion
  "stagnant",
  // No meaningful progress
  "regressing"
  // Getting worse or going in circles
]);
var ProgressAnalysisSchema = external_exports.object({
  trend: ProgressTrendSchema,
  consecutiveFailures: external_exports.number(),
  totalAttempts: external_exports.number(),
  uniqueFingerprints: external_exports.number(),
  recommendation: external_exports.string().optional()
});
var LimitCheckResultSchema = external_exports.object({
  exceeded: external_exports.boolean(),
...
var LedgerManager = class {
  ...
  async initialize() {
    ...
    if (existsSync2(this.ledgerPath)) {
      try {
        const rawData = readFileSync2(this.ledgerPath, "utf-8");
        const parsed = JSON.parse(rawData);
        this.ledger = TaskLedgerSchema.parse(parsed);
        this.ledger.updatedAt = (/* @__PURE__ */ new Date()).toISOString();
        await this.save();
        return;
      } catch (error) {
        if (this.config.storage.archiveCorrupted) {
          const archivePath = join2(
  ...
  async updateDirectiveStatus(directiveId, status) {
    ...
    if (status === "completed") {
      directive.completedAt = (/* @__PURE__ */ new Date()).toISOString();
    }
    await this.save();
  }
  /**
...
import { existsSync as existsSync4, readFileSync as readFileSync3, readdirSync } from "node:fs";
...
function findCommandFile(commandsDir, commandName) {
  if (!existsSync4(commandsDir)) return null;
  const mdPath = join4(commandsDir, `${commandName}.md`);
  if (existsSync4(mdPath)) return mdPath;
  const plainPath = join4(commandsDir, commandName);
  if (existsSync4(plainPath)) return plainPath;
  if (commandName.includes(":")) {
    const nestedPath = commandName.replace(/:/g, "/");
    const nestedMdPath = join4(commandsDir, `${nestedPath}.md`);
    if (existsSync4(nestedMdPath)) return nestedMdPath;
  }
  return null;
}
...
function expandFileReferences(content, projectDir) {
  ...
  const expanded = content.replace(fileRefPattern, (match, filePath) => {
    ...
    try {
      const fileContent = readFileSync3(absolutePath, "utf-8");
...
function extractFileReferences(prompt, projectDir) {
  const files = [];
  const fileRefPattern = /@([^\s,;:'"<>()[\]{}]+)/g;
  let match;
...
async function expandSlashCommand(prompt, projectDir) {
  ...
  const projectCommandsDir = join4(projectDir, ".claude", "commands");
  const userCommandsDir = join4(homedir(), ".claude", "commands");
  ...
  const rawContent = readFileSync3(commandFile, "utf-8");
  ...
  const summary = firstLine.startsWith("#") ? firstLine.replace(/^#+\s*/, "") : `Command: ${name}`;
  return {
...
var UserPromptSubmitRawInputSchema = external_exports.object({
...
function determineDirectiveType(prompt, isFirstPrompt) {
...
async function handleUserPromptSubmit(rawInput) {
...
var PostToolUseRawInputSchema = external_exports.object({
  hook_event_name: external_exports.literal("PostToolUse"),
  tool_name: external_exports.string(),
  tool_input: external_exports.unknown(),
  tool_output: external_exports.unknown().optional(),
  session_id: external_exports.string(),
...
async function handlePostToolUse(rawInput) {
...
async function readTranscript(transcriptPath, options) {
  if (!existsSync5(transcriptPath)) {
    return [];
  }
  const fileContent = readFileSync4(transcriptPath, "utf-8");
...
`);
    ...
    sections.push("");
    if (diff.patch && diff.patch.length < 1e4) {
      sections.push("Patch:");
      sections.push("```diff");
  ...
  if (!lastMessage) {
    ...
    const truncated = lastMessage.length > 2e3 ? lastMessage.substring(0, 2e3) + "\n... (truncated)" : lastMessage;
      ...
      try {
        ...
        if (parsed.result !== void 0) {
          ...
          if (textBlock && typeof textBlock.text === "string") {
            try {
              resolve2(JSON.parse(textBlock.text));
            } catch {
              resolve2(textBlock.text);
            }
          } else {
            resolve2(parsed);
          }
...
function createFailOpenVerdict(errorMessage) {
  return {
    pass: true,
...
// src/hooks/stop.ts
var StopHookRawInputSchema = external_exports.object({
  hook_event_name: external_exports.literal("Stop"),
  session_id: external_exports.string(),
  transcript_path: external_exports.string(),
  cwd: external_exports.string(),
...
async function handleStop(rawInput) {
...
var SessionStartRawInputSchema = external_exports.object({
  hook_event_name: external_exports.literal("SessionStart"),
  session_id: external_exports.string(),
  transcript_path: external_exports.string(),
  cwd: external_exports.string(),
  source: external_exports.enum(["startup", "resume", "clear", "compact"]).optional()
});
async function handleSessionStart(rawInput) {
  ...
  if (source === "clear") {
    const activeDirectives = ledger.getActiveDirectives();
    if (activeDirectives.length > 0) {
      const directivesList = activeDirectives.map((d, i) => `${i + 1}. ${d.normalizedIntent || d.rawText}`).join("\n");
      return {
